...
ui <- dashboardPage(
  ...
  skin="black",
  dashboardHeader(title = "SUM|VEM-v1.0.2"),
...
server <- function(input, output) {
  ...
  quali <- reactive({
    ...
    if (sum() < SUM.RANGE[1]) {
      text <- c(paste('Severely reduced vocal capacity (SUM < ',SUM.RANGE[1],'), Dynamic range: ',DR,' half-tones',sep=''),'#e50000')
    }
    if (sum() >= SUM.RANGE[1] & sum() < SUM.RANGE[2]) {
      text <- c(paste('Moderately reduced vocal capacity (',SUM.RANGE[1],' <= SUM < ',SUM.RANGE[2],'), Dynamic range: ',DR,' half-tones',sep=''),'#ffcccb')
    }
    if (sum() >= SUM.RANGE[2] & sum() < SUM.RANGE[3]) {
      text = c(paste('Mildly reduced vocal capacity (',SUM.RANGE[2],' <= SUM < ',SUM.RANGE[3],'), Dynamic range: ',DR,' half-tones',sep=''),'#90ee90')
    }
    if (sum() > SUM.RANGE[3]) {
      text <- c(paste('Normal vocal capacity (SUM >= ',SUM.RANGE[3],'), Dynamic range: ',DR,' half-tones',sep=''),'#15b01a')
    }}
    text
  })
